...
const char *TARGET_DOMAIN_FILE = "targetDomain";
const char *FIREFOX_INSTALL_PATH = "firefoxPath";
...
NSString *getBundleSpecifiedFirefoxPath();
...
int main(int argc, char **argv)
{
  ...
  if (!firefoxPath) {
    firefoxPath = getBundleSpecifiedFirefoxPath();
    if (!firefoxPath) {   
...
char *getBundleSingleLineFile(const char *filename)
{
  char targetFile[4096];
  snprintf(targetFile, 4096, "%s/Contents/%s", [[[NSBundle mainBundle] bundlePath] UTF8String], filename);
  FILE *fp;
...
char *getAppTargetDomain()
{
  return getBundleSingleLineFile(TARGET_DOMAIN_FILE);
}

NSString *getBundleSpecifiedFirefoxPath()
{
  char *install= getBundleSingleLineFile(FIREFOX_INSTALL_PATH);
  if (install) {
    return [[NSString alloc] initWithCString:install];
  }
  return NULL;
}
